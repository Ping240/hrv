...
if __name__ == "__main__":
    os.makedirs("saved_models/onestep_10fold_2class_odownsampled", exist_ok=True)

    all_preds, all_labels = [], []
    fold_accuracies = []

    # skf = KFold(n_splits=10, shuffle=True, random_state=42)
    skf = KFold(n_splits=10, shuffle=True, random_state=99)
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_all, y_all), 1):
        ...
        # 儲存最佳模型
        torch.save(best_state_dict, f"saved_models/onestep_10fold_2class_odownsampled/fold{fold}_best.pth")
        print(f"儲存模型至: saved_models/onestep_10fold_2class_odownsampled/fold{fold}_best.pth (Best Train Acc: {best_acc:.2f}%)")
    ...
    print("分類報告:\n", classification_report(all_labels, all_preds, target_names=['Baseline/Amusement', 'Stress']))
