...
if __name__ == "__main__":
    os.makedirs("saved_models/onestep_2class", exist_ok=True)
    ...
    for i, test_subject in enumerate(subjects):
        ...
        y_train = train_df['EmotionBinary'].values
        X_test = test_df.iloc[:, 2:2562].values
        y_test = test_df['EmotionBinary'].values
        ...
        # 儲存最佳模型
        torch.save(best_state_dict, f"saved_models/onestep_2class/emotion_{test_subject}.pth")
        print(f"儲存模型至: saved_models/onestep_2class/emotion_{test_subject}.pth (Best Train Acc: {best_acc:.2f}%)")
