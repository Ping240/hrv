...
#     def __init__(self, num_classes=3, hidden_dim=32):
#         super().__init__()
#         input_dim = num_classes * 3  # 3 sets of softmax outputs
...
#     def forward(self, probs_os, probs_ts, probs_hrv):
#         x = torch.cat([probs_os, probs_ts, probs_hrv], dim=1)  # [batch, 9]
...
for fold, (train_idx, test_idx) in enumerate(logo.split(X_all, y_emotion, groups=subjects)):
    ...
    # probs_os, probs_ts, probs_hrv shape: [batch_size, num_classes]
...
print_per_class_accuracy(all_labels, all_preds_fuse, class_names=["amusement", "baseline", "stress"])
...
print_per_class_accuracy(all_labels, all_preds_os, class_names=["amusement", "baseline", "stress"])
...
print_per_class_accuracy(all_labels, all_preds_hrv, class_names=["amusement", "baseline", "stress"])
...
print_per_class_accuracy(all_labels, all_preds_ts, class_names=["amusement", "baseline", "stress"])
